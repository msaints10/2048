...
;;lectura de la ip a utilizar para conectarse con el server
(display "Coloque la IP que utilizará: ")
(define IPdress (read-line))
;;definicion de valores de entrada y salida para comunicacion con el server
(define-values (in out) (tcp-connect IPdress 1150))
;;se crea el plano
(open-graphics)
(define plano (open-viewport "2048 - Blood Dragon - 1 vs 1 online - Jugador 2" 1000 700))
(((draw-pixmap-posn "fondo_cliente_2048.jpg") plano) (make-posn 0 0) "white")
;;definicion del procedimiento que inicia el juego del usuario
(define (client_player_2048 base_user)
  ...
  (define (console-plano)
    ...
    (write (string-append "1-" (number->string(buscarMaximo))) out)
    (flush-output out)
    )
  ...
  (define (contador_movimientos)
    ...
    (write (string-append "2-" (number->string vcontador_movimientos)) out)
    (flush-output out)
    )
  ...
  (define (verifica_vacias)
    ...
    (write (string-append "3-" (number->string aux)) out)
    (flush-output out)
    )
    ...
    (cond
      [(= opcion 1)
       (begin
         ...
         (write (string-append "4-" "Izquierda") out)
         (flush-output out)
         )]
      [(= opcion 2)
       (begin
         ...
         (write (string-append "4-" "Derecha") out)
         (flush-output out)
         )]
      [(= opcion 3)
       (begin
         ...
         (write (string-append "4-" "Abajo") out)
         (flush-output out)
         )]
      [(= opcion 4)
       (begin
         ...
         (write (string-append "4-" "Arriba") out)
         (flush-output out)
         )]
      )
    )
  ...
  (define (principio)    
    ...
    (write (string-append "2-" (number->string 0)) out)
    (flush-output out)
    (verifica_vacias)
...
;;se define la base recibida por el server para iniciar la partida del cliente
(define base_recibida (read in))
...
(define (escuchar_server)
  (let ciclo_escuchar
    ...
    (define msg (read in))
    (cond
      ((equal? (substring msg 0 2) "1-") ;; 1- nos indicara el mayor del contrincante
       (begin
         ((draw-solid-rectangle plano) (make-posn 840  595) 100 20  "white")
         ((draw-string plano) (make-posn 840 605) (substring msg 2 (string-length msg)) "black")
         (ciclo_escuchar)
         )
       )
      ((equal? (substring msg 0 2) "2-") ;; 2- nos indicara el numero de movimientos realizados por el contrincante
       (begin
         ((draw-solid-rectangle plano) (make-posn 840 550) 100 20  "white")
      ...
      ((equal? (substring msg 0 2) "4-") ;; 4- nos indicara el movimiento elegido por el contrincante
       (begin
